...
export default function PrivacyPage() {
  const t = useT();
  const sections = t('content.privacy.sections');
      ...
      <div className="max-w-2xl mx-auto px-4 py-12 space-y-8">
        <div className="flex items-center gap-3 mb-8">
          <LocaleLink href="/" className="text-red-400 hover:text-red-300 transition-colors text-sm font-bold">
            {t('content.privacy.backHome')}
          </LocaleLink>
        </div>

        <h1 className="text-3xl font-black text-heading">{t('content.privacy.pageTitle')}</h1>
        <p className="text-xs text-muted">
          {t('content.privacy.lastUpdated')}: {t('content.privacy.lastUpdatedDate')}
        </p>
