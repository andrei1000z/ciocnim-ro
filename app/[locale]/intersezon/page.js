...
function formatEasterDate(d, locale) {
  const months = {
    ro: ['ianuarie', 'februarie', 'martie', 'aprilie', 'mai', 'iunie', 'iulie', 'august', 'septembrie', 'octombrie', 'noiembrie', 'decembrie'],
    en: ['January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December'],
  };
  const m = (months[locale] || months.ro)[d.getMonth()];
  return `${d.getDate()} ${m} ${d.getFullYear()}`;
}

export default function IntersezonPage() {
  const router = useRouter();
  const { nume, isHydrated } = useGlobalStats();
  const [mounted, setMounted] = useState(false);
  const [countdown, setCountdown] = useState(null);
  const [phase, setPhase] = useState("weekAfter"); // weekAfter | preparing
  ...
  useEffect(() => {
    ...
    const tick = () => {
      const now = Date.now();
      const inTransition = now < TRANSITION_END_TS;
      setPhase(inTransition ? "weekAfter" : "preparing");
  ...
  return (
    <div className="bg-main text-body min-h-screen">
      <PageHeader />
      ...
      <main className="w-full max-w-2xl mx-auto pt-8 pb-16 px-4 space-y-10">
        ...
        {/* BOT PLAY — joacă simplu cu un bot, fără clasament */}
        <motion.section
          initial={{ opacity: 0, y: 10 }}
        ...
        >
          ...
          {mounted && easter2027 && (
            <p className="text-xs text-dim mb-3">{formatEasterDate(easter2027, "ro")}</p>
          )}
          {countdown && (
