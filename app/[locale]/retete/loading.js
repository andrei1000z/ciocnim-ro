    ...
    <main className="min-h-screen bg-main text-body">
      ...
      <div className="w-full max-w-3xl mx-auto pt-8 pb-16 px-6 space-y-6">
        <div className="text-center space-y-3">
          <div className="h-10 w-56 bg-elevated rounded-xl mx-auto animate-pulse" />
          <div className="h-4 w-72 bg-elevated rounded mx-auto animate-pulse" />
        </div>
        <div className="grid gap-6 sm:grid-cols-2">
          {[...Array(4)].map((_, i) => (
            <div key={i} className="bg-card border border-edge rounded-2xl p-6 h-48 animate-pulse" style={{ animationDelay: `${i * 100}ms` }} />
          ))}
        </div>
      </div>
    </main>
