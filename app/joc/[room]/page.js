...
/**
 * ========================================================================================================================
 * CIOCNIM.RO - ARENA DE LUPTĂ (V30.1 - TRADIȚIONAL PREMIUM & BULLETPROOF IMPACT)
 * ========================================================================================================================
 */
...
const OuTitan = ({ skin, spart = false, hasStar = false, isGolden = false }) => {
  ...
  return (
    <div className={`relative transition-all duration-700 ${!spart ? 'animate-float-v9' : 'scale-[0.85] opacity-70 rotate-6 filter grayscale-[30%]'}`} style={{ width: 'clamp(110px, 30vw, 180px)', height: 'auto', aspectRatio: '1 / 1.35' }}>
      ...
      {spart && <div className="absolute inset-0 flex items-center justify-center text-7xl md:text-9xl animate-pop pointer-events-none z-30 drop-shadow-[0_0_40px_rgba(220,38,38,0.8)] filter sepia-[0.3]">💥</div>}
    </div>
  );
};
...
function ArenaMaster({ room }) {
  ...
  useEffect(() => {
    ...
    const botTimeout = setTimeout(() => {
      setIsBotMatch(true);
      const botName = "🤖 ADVERSAR (BOT)";
      setOpponent({ jucator: botName, skin: 'gold', isGolden: false, hasStar: true, regiune: "România" });
  ...
  useEffect(() => {
      if (isBotMatch && atacantName === "🤖 ADVERSAR (BOT)" && !rezultat && !isStriking) {
          const timeout = setTimeout(() => {
              executeBattle({ castigaCelCareDa: Math.random() < 0.5, atacant: "🤖 ADVERSAR (BOT)" });
              incrementGlobal(); 
    ...
    <>
      <div className={`w-full max-w-4xl flex flex-col items-center gap-8 pt-12 ${impactFlash ? 'animate-impact scale-105 blur-[2px]' : ''}`}>
        ...
        {isPrivate && !isProvocare && (
          <button onClick={copyRoomCode} className="group relative bg-[#0a0505]/90 backdrop-blur-xl px-8 py-4 rounded-full border border-red-900/40 mb-4 shadow-[0_10px_30px_rgba(0,0,0,0.8)] hover:bg-[#140a0a] hover:border-red-500/50 transition-all active:scale-95 z-10 overflow-hidden">
            <div className="absolute inset-0 bg-[url('/pattern-wood.png')] opacity-10 mix-blend-overlay pointer-events-none"></div>
            <div className="flex items-center gap-3 relative z-10">
              <span className="text-[10px] font-black uppercase tracking-[0.3em] text-amber-500/50 group-hover:text-amber-500/80 transition-colors">Cod Cameră: </span>
              <span className="text-amber-500 font-black text-2xl tracking-widest drop-shadow-[0_0_15px_rgba(245,158,11,0.5)]">{room.replace('privat-', '')}</span>
              <span className="bg-red-900/30 p-2 rounded-xl text-xs ml-2 group-hover:bg-red-900/50 transition-all border border-red-900/50">{copied ? '✅' : '📋'}</span>
            </div>
        ...
        <div className="flex justify-between items-center w-full px-4 md:px-20 mt-4">
          ...
          <div className="flex flex-col items-center gap-6 flex-1">
            ...
            <div className="bg-[#140a0a]/90 backdrop-blur-md p-4 px-6 rounded-2xl text-center border border-red-900/40 border-l-4 border-l-green-500 relative min-w-[140px] shadow-[0_10px_30px_rgba(0,0,0,0.5)] overflow-hidden">
              <div className="absolute inset-0 bg-[url('/pattern-wood.png')] opacity-10 mix-blend-overlay pointer-events-none"></div>
              {atacantName === nume && !rezultat && opponent && <span className="absolute -top-3 left-1/2 -translate-x-1/2 bg-red-700 text-[10px] font-black uppercase tracking-[0.3em] px-3 py-1 rounded-full shadow-[0_5px_15px_rgba(220,38,38,0.5)] border border-red-500/50">ATACĂ</span>}
              <span className="text-[9px] font-black uppercase tracking-widest text-amber-500/50 block mb-1 truncate relative z-10">{userStats.regiune || "Muntenia"}</span>
              <span className="text-xl font-black text-white italic drop-shadow-sm relative z-10">{nume}</span>
            </div>
          </div>

          <div className="text-4xl md:text-5xl font-black text-amber-500/20 italic drop-shadow-sm px-4 filter sepia-[0.5]">VS</div>
          ...
          <div className="flex flex-col items-center gap-6 flex-1 text-center">
            {opponent ? (
              <motion.div initial={{ opacity: 0, scale: 0.5 }} animate={{ opacity: 1, scale: 1 }} className="flex flex-col items-center gap-6">
                ...
                <div className="bg-[#140a0a]/90 p-4 px-6 rounded-2xl border border-red-900/40 border-r-4 border-r-red-600 backdrop-blur-md relative min-w-[140px] shadow-[0_10px_30px_rgba(0,0,0,0.5)] overflow-hidden">
                  <div className="absolute inset-0 bg-[url('/pattern-wood.png')] opacity-10 mix-blend-overlay pointer-events-none"></div>
                  {atacantName === opponent.jucator && !rezultat && <span className="absolute -top-3 left-1/2 -translate-x-1/2 bg-red-700 text-[10px] font-black uppercase tracking-[0.3em] px-3 py-1 rounded-full shadow-[0_5px_15px_rgba(220,38,38,0.5)] border border-red-500/50">ATACĂ</span>}
                  <span className="text-[9px] font-black uppercase tracking-widest text-red-500/60 block mb-1 truncate relative z-10">{opponent.regiune || "Necunoscut"}</span>
                  <span className="text-xl font-black text-white italic drop-shadow-sm relative z-10">{opponent.jucator}</span>
                </div>
              </motion.div>
            ) : (
              <div className="w-[140px] h-[180px] bg-[#0a0505]/80 rounded-[3rem] border-2 border-dashed border-red-900/40 animate-pulse flex items-center justify-center text-[10px] font-bold tracking-widest uppercase text-amber-500/40 text-center px-4 mt-[30px] backdrop-blur-sm shadow-inner relative overflow-hidden">
        ...
        <div className="w-full max-w-sm px-4 mt-8 z-10">
          {opponent && !rezultat && (
            <button 
              ...
              className={`w-full py-6 rounded-[2rem] font-black uppercase tracking-[0.3em] text-sm md:text-base transition-all shadow-lg overflow-hidden relative ${
                canStrike && !isStriking 
                  ? 'bg-red-700 text-white shadow-[0_20px_40px_rgba(220,38,38,0.4)] border-2 border-red-500/50 hover:bg-red-600 hover:scale-[1.02] active:scale-95 animate-pulse' 
                  : 'bg-[#140a0a] text-white/30 cursor-not-allowed border-2 border-red-900/30 backdrop-blur-md'
              }`}
            >
              <div className="absolute inset-0 bg-[url('/pattern-wood.png')] opacity-10 mix-blend-overlay pointer-events-none"></div>
              <span className="relative z-10">{isStriking ? "⏳ SE CIOCNEȘTE..." : (canStrike ? "💥 CIOCNEȘTE OUL!" : "🛡️ APĂRĂ OUL!")}</span>
            </button>
          )}
        </div>

        {/* CHAT REDESIGN (Tradițional Premium) */}
        <div className="w-full max-w-md bg-[#0a0505] border-2 border-red-900/40 p-5 md:p-6 rounded-[2.5rem] mt-4 z-10 shadow-[0_30px_60px_rgba(0,0,0,0.8)] relative overflow-hidden">
          <div className="absolute inset-0 bg-[url('/pattern-wood.png')] opacity-5 mix-blend-overlay pointer-events-none"></div>
          ...
          <div className="h-36 overflow-y-auto flex flex-col-reverse gap-3 mb-4 custom-scrollbar pr-2 relative z-10">
            {messages.map((m, i) => (
              <div key={i} className={`flex flex-col ${m.autor === nume ? 'items-end' : 'items-start'}`}>
                <span className="text-[8px] font-black uppercase tracking-[0.3em] text-amber-500/50 px-2 mb-1">{m.autor}</span>
                <div className={`px-4 py-2.5 rounded-2xl text-xs md:text-sm font-bold shadow-sm border ${m.autor === nume ? 'bg-red-700 text-white rounded-tr-sm border-red-500/50' : 'bg-[#140a0a] text-white/90 rounded-tl-sm backdrop-blur-md border-red-900/30'}`}>
                   {m.text}
                </div>
              </div>
            ))}
            {messages.length === 0 && (
                <div className="text-center w-full h-full flex flex-col justify-center items-center opacity-40 mt-8">
                    <span className="text-2xl mb-2 filter sepia-[0.3]">💬</span>
                    <span className="text-[10px] font-black uppercase tracking-[0.3em] text-amber-500/70">Liniște la masă...</span>
                </div>
            )}
          </div>
          ...
          <div className="flex gap-2 bg-[#140a0a] p-1.5 rounded-full border border-red-900/40 focus-within:border-red-500/50 focus-within:bg-[#1a0f0f] transition-all relative z-10 shadow-inner">
            <input 
               ...
               className="flex-1 bg-transparent pl-5 text-xs font-black outline-none text-white tracking-widest placeholder:text-amber-500/30" 
            />
            <button onClick={handleChat} className="bg-red-900/30 w-12 h-12 rounded-full hover:bg-red-700 transition-colors border border-red-900/50 text-sm active:scale-95 shadow-md">🕊️</button>
          </div>
        </div>
      </div>
        ...
        {rezultat && (
          ...
          >
            <motion.div 
              ...
              className={`max-w-md w-full bg-[#0a0505] p-8 md:p-10 rounded-[3rem] border-2 shadow-[0_50px_100px_rgba(0,0,0,0.9)] relative overflow-hidden pointer-events-auto ${rezultat.win ? 'border-green-600/40' : 'border-red-600/40'}`}
            >
              <div className="absolute inset-0 bg-[url('/pattern-wood.png')] opacity-10 mix-blend-overlay pointer-events-none"></div>
              ...
              <motion.div variants={{ hidden: { opacity: 0, scale: 0 }, visible: { opacity: 1, scale: 1 } }} className="text-8xl md:text-9xl drop-shadow-[0_0_30px_rgba(255,255,255,0.2)] relative z-10 mb-4 filter sepia-[0.3]">
                {rezultat.win ? '👑' : '🥀'}
              </motion.div>
              
              <motion.h2 variants={{ hidden: { opacity: 0, y: 20 }, visible: { opacity: 1, y: 0 } }} className={`text-4xl md:text-5xl font-black italic tracking-tighter drop-shadow-lg uppercase relative z-10 ${rezultat.win ? 'text-green-500' : 'text-red-500'}`}>
              ...
              <motion.div variants={{ hidden: { opacity: 0, y: 20 }, visible: { opacity: 1, y: 0 } }} className="relative mt-8 mb-8 z-10 pointer-events-none">
                 <div className={`absolute -top-3 left-1/2 -translate-x-1/2 bg-[#0a0505] px-4 text-[10px] font-black uppercase tracking-[0.3em] z-10 border rounded-full ${rezultat.win ? 'text-green-500/70 border-green-900/50' : 'text-red-500/70 border-red-900/50'}`}>
                   {rezultat.win ? 'Învățătură' : 'Alinare'}
                 </div>
                 <div className="bg-[#140a0a] border border-red-900/30 p-6 md:p-8 rounded-3xl shadow-inner relative overflow-hidden">
                   ...
                   <span className="absolute top-2 left-4 text-5xl text-amber-500/10 font-serif leading-none">"</span>
                   <p className="text-sm md:text-base font-bold text-amber-500/80 italic leading-relaxed relative z-10 mt-2 drop-shadow-sm">
                     {citatFinal}
                   </p>
                   <span className="absolute bottom-[-15px] right-4 text-5xl text-amber-500/10 font-serif leading-none">"</span>
                 </div>
              </motion.div>
              ...
              <motion.div variants={{ hidden: { opacity: 0, y: 20 }, visible: { opacity: 1, y: 0 } }} className="flex flex-col gap-4 relative z-50">
                <button 
                  ...
                  className="w-full bg-[#140a0a] hover:bg-red-900/30 py-5 rounded-[2rem] font-black uppercase tracking-[0.3em] text-sm transition-all active:scale-95 border-2 border-red-900/50 shadow-sm cursor-pointer relative z-50 pointer-events-auto text-white"
                >
...
export default function PaginaJoc({ params }) {
  ...
  return (
    <main className="min-h-screen w-full bg-[#050202] text-white flex flex-col items-center justify-center relative overflow-hidden pattern-tradition">
      ...
      <Suspense fallback={<div className="font-black animate-pulse text-amber-500/70 tracking-widest text-sm uppercase drop-shadow-sm">AȘEZĂM MASA...</div>}>
